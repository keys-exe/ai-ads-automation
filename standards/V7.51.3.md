...
**Version 7.51.3 — supersedes all prior versions.** *(three modes — Realistic, 3D Pixar, Claymation; three image models only — `nano_banana_pro`, `nano_banana_2`, `gpt_image_2_5` Sunburst; the dwelling is an object — Property Standard at §30G)*
...
Three visual registers. Never mixed in one project unless a hybrid is explicitly requested.
...
The three are **not interchangeable.** Mode 1 = true, found, phone-real. Mode 2 = designed, polished, sincere, warm. Mode 3 = handmade, tactile, visibly physical. Pick on audience, character type and emotional register.
...
- No real-camera or photographic language inside Mode 2. Mode 3 is a photograph of a real miniature, so its capture language is the tabletop rig (§24F), never a phone.
...
**Default is Short VSL or UGC Ad.** Long VSL only on explicit request.
...
|---|---|---|
| **Any beat carrying a readable wordmark** — hero product, held product, worn with the face toward camera, pack and guarantee | `gpt_image_2_5`, `variant: sunburst`, `quality: high`, `resolution: 2k`, product reference under `image_references`, `REF-PROD` in prose exactly as on any other call | **Sanctioned challenger to `nano_banana_pro`.** Becomes the default on typed beats only when the first side-by-side holds both the wordmark *and* the §22A/§22T register |
...
| Candid B-roll face seeds (§22T) | `nano_banana_pro` · **Sunburst**, chosen at §18A | A model that cleans the file is off the Mode 1 route regardless of skin quality — checked on every batch's first frame |
...
| Volume B-roll, no type | `nano_banana_2` · **Sunburst**, chosen at §18A | — |
| **Any Mode 1 beat class except mechanism** | `gpt_image_2_5` **Sunburst** — chosen per build at §18A | **In the arsenal (V7.50.0, user decision).** Test pairs run 17 Sep 2026 — kitchen, park, park with tag line, living room with reference, talking-head seed, worn front beat — logged below in Open Decisions |
...
| Mechanism A–C | **Kling-direct at 3s** | The 3s floor exists nowhere else; §12A's compressed blocks are designed to it |
| Lifestyle B-roll | Kling by default; **Wan 3.0 or Seedance 2.5 in references mode as sanctioned alternates** | Audio is discarded, so the voice argument does not apply; a B-roll beat can move routes without a continuity cost. Use the alternates to break a stubborn beat (§5's routing fallback, now with two siblings), to spend the 2–30s range on a demonstration that must play uncut through the proof (§30B), or wherever a recurring subject's face must show small or turning — references mode establishes the face from the sheet, not the seed |
| Talking heads on Wan / Seedance | **Blocked until the voice-lock test passes** (Open Decisions) | The audio-reference slot is the reason to want it; until it is shown to carry timbre, a prose-model talking head is a second voice for the character |
...
### References mode — the default on Wan and Seedance *(V7.49.4 — unverified, the standard is written so the first test has something to test)*
...
**Four is the ceiling.** Both vendors allow far more and both vendor guides say the same thing: start with one or two and add only what a test proves. Five product angles do not make the product more consistent; they make it ambiguous which one wins. One canonical image per object (§5).
...
I2V   Kling AI → kling-video-v3_0_omni      (image_1 = that URL)
      ...
      Seedance 2.5 image-to-video           (image_urls = [that URL])                 — unverified
```
...
**Anatomy B-roll is 3 seconds, including the whole-arc beat** *(locked V7.48)*. The compressed blocks are the default form and the full-length blocks are the exception for a beat with room. The prior "the arc needs full duration, never a 2–3s cutaway" clause is **withdrawn**: it was inferred from a reference clip that happened to be five seconds long, and the duration was never part of the finding. Where a problem-to-relief line needs covering, two forms are equally valid and picked per line at the act map — `ANAT-ARC-S`, where the change happens inside one shot with no edit point mid-line, or a **match-cut pair** sharing camera position, framing and cadence, where the change lands exactly on the cut and the editor controls its timing.

**Generate mechanism beats at the shortest duration the model offers.** A travelling boundary spread over five seconds has no urgency; the same event in three is a hook. §4's Kling-direct route offers 3–15s granularity where the Higgsfield floor is 5 — **generate mechanism beats at 3s on Kling-direct** and the escalation is designed edge-to-edge with nothing wasted.
...
Work runs continuously. Steps 3, 4 and 5 **send** their artefact and continue in the same pass — a handoff, not an approval. **The only gate in the build is step 6.**
...
| 6 | **Hooks, one by one** | AC → **HG** | Serial: deliver → generate → first-frame check (§5/§30C/§30E) → confirm → next. Seeds built against plates that already exist. Confirmed renders feed the Scene and Subject Registries as they land. **The only gate** |
...
One mode per build: **Mode 1 Realistic**, **Mode 2 3D Pixar**, or **Mode 3 Claymation**. A hybrid is declared per act here, never discovered at the act map (§2). A realistic reference selects Mode 1 automatically (§22).
...
| Readable wordmark | `nano_banana_pro` · Sunburst |
| Candid face seeds (§22T) | `nano_banana_pro` · Sunburst |
| Talking-head seeds | `nano_banana_pro` · Sunburst |
| Volume B-roll, no type | `nano_banana_2` · Sunburst |
...
**Mode 2 — 3D Pixar** and **Mode 3 — Claymation**
...
2. **Reasoning-model guard on Mode 1.** Sunburst reasons. `CAP-FILE`'s final clause and `NEG-FILE` are stated in full on every Mode 1 call to either. The first-frame check adds: *has the model lit, composed or cleaned anything the prompt said nobody did?* If yes, the class reroutes and the finding is recorded.
...
6. **Mode 2 and Mode 3 are locked to Nano Banana.** `gpt_image_2_5` is never routed in either mode, for any beat class. Not a default the step-2 read can override. A hybrid build takes the full menu on its Mode 1 acts only, and the lock table records the split by act.
...
Close-up matches the front panel · hair tone identical in all five · window on the same side in all five, profiles lit from opposite sides · wardrobe identical, present in every panel · nothing on the skin in one panel only · close-up texture holds under zoom. One failing panel is a reroll of the sheet, not a note: a wrong panel becomes a wrong reference on some beat later.
...
**The four exempt classes, and they are a list rather than an inference:**
...
**Scope:** Mode 1 talking heads only. Not B-roll (audio discarded under VO), not mechanism beats, not hero product, **not CCTV** — that register is silent by default and its sound is built in post (§22E).
...
**Scope:** every Mode 1 T2I containing a human, **including MOUNT-CLEAN beats** (§22E) — an elevated camera is still a phone file and skin still resolves. Dropped only on CCTV-FULL. Lives in **T2I only** (§6 — a property of the frame); I2V inherits via `INHERIT-SUBJ`. Nothing enters the I2V budget.
...
Skin-critical beats are shot **tighter than the composition wants**, and the wide framing is recovered at the video stage or in the crop. A tight close-up — cropped through the forehead and at the collarbone, the face filling the frame — puts several times the pixels on the same skin as a chest-up frame at identical resolution. Where the beat cannot be recomposed, raise resolution instead; where neither is available, the beat cannot carry a skin claim and the act map should not have given it one.
...
**Scope:** every Mode 1 lifestyle B-roll T2I — POV, propped candid, second person, walking phone, incidental. Not talking heads (the §22S stack is unchanged there), not mechanism, not hero product, not CCTV.
...
**3. Face-subject beats seed medium-close.** The face fills the top half of the frame. If the line also needs the room, the wide is **a second beat, the establishing one**, and it does not carry the skin — a face at chest-up across a desk is ~350 pixels tall at 9:16/2k and cannot resolve pore-level relief whatever the prompt says. This is an act-map decision (§30B Part 1), not a prompt decision.
...
**Scope: every I2V, all modes.** Lives in `motion` and `negatives`. **Nothing enters T2I** — distortion is change over time, and a start frame has none.
...
**Steps 1–5 are one delivery. The build stops once, at step 6.**
...
- `camera.framing`: documentation register (§15) **plus this beat's frame side and framing step** (§30A) — framing only, never movement
...
- `camera.framing`: must state hands are visible in the lower frame, or gestures have nowhere to land. On a held-product beat, must state the product is inside frame
...
**38. Call parameters → 9:16, and duration by the E6 function.** Aspect 9:16 unless the Build Sheet overrides; B-roll calls 5s, mechanism 3s on Kling-direct, dialogue beats per the words→duration table; parameters live at call level, never inside B-roll JSON (§27, Appendix E6–E7).
...
One row per beat: `beat_id · phrase_ids[] · t2i_prompt_path · t2i_job_id · t2i_status · t2i_qa (pass/fail per check) · i2v_prompt_path · i2v_job_id · i2v_status · i2v_qa · retries[{class, action, ts}] · attachments[{role, media_or_job_id}] · delivered (bool) · reissue_flag (standard §, reason)`. Plus build-level: `property{dwelling_id → {sheet_path, plate_job_id, plate_check}}` · `plates{location→job_id}` (every location row carrying a `dwelling_id` or an explicit null) · `subjects{S-id→job_id}` · `story_days{day→outfit_row}` · `capture_events{event_id→{story_day, location_id, beats[]}}` · `declared{side, mechanism_claim, format, mode, model_lock{beat_class → model, variant, quality}}` · `version_built_against`. The ledger is the source for §34 global scans, reissue passes, resume-after-interruption, and the Open Decisions counts — **computed, never hand-maintained**.
...
Words at pace → duration: brisk ≤9 → 5s · unhurried ≤8 → 5s · brisk 10–20 → 10s · unhurried 9–18 → 10s · above → split the line per §29. Talking-while-doing always reads the unhurried column. B-roll calls: 5s (the Higgsfield floor; Wan 2–30s and Seedance 4–30s where routed there, still 5s unless the demonstration needs longer); mechanism on Kling-direct: 3s; `ANAT-STRESS-R` and whole-arc beats: 5s minimum. Aspect: 9:16 unless the Build Sheet overrides. Resolution per §4.
...
**I2V (Seedance 2.5, omni-reference — the default, unverified):** omni-reference endpoint → `{prompt: <REF-MANIFEST + prose>, images_list: [<seed>, <character sheet>, <product ref>, <scene plate if PLATED>], audios_list: [<voice clip>] under the same condition, resolution: "1080p", aspect_ratio: "9:16", duration: <E6, never "auto">}`. Audio is generated regardless and discarded on B-roll.
**I2V (Seedance 2.5, first-frame fallback):** image-to-video endpoint → `{prompt, image_urls: [<seed URL>], resolution, aspect_ratio, duration}`. One image only — a second image is a last frame.
...
**Visual-check, not counted** — §30G, §24A, §24B, §24C, §24D, §24E, the two unverified Location Profiles (with the skin-under-overcast check), the §30B register gate, the §9A-P inner-face read, plus the visual checks recorded above (§12B, §27C, the ANAT-STRESS pair, the §30C scene hold, and §30E's subject-plate and axis reads). They sit here until someone generates one and looks — the count is whatever the list says, computed, never hand-maintained.

---

# CHANGELOG — V7.51.2 → V7.51.3 *(cut authorised)*

| § | Change |
|---|---|
| **4, 18A** | **The arsenal is three models.** `nano_banana_pro`, `nano_banana_2`, `gpt_image_2_5` Sunburst. **GPT Image 2.5 Flare retired** — Sunburst is the better frame and a second variant had to be reasoned about at every beat class. **`nano_banana_flash` retired** — it was never a route, only the symptom of the §5 fault, and it is now named as banned so it cannot be reached for under time pressure |
| 4 | Because Flare is the *catalogue default*, `variant: sunburst` is now passed explicitly on every call without exception — an omitted variant silently runs the retired one |
| **5** | **A logged `nano_banana_flash` or `flare` is a failed generation.** Discarded and re-run in the platform interface; never accepted because the frame looks good, never entered in the ledger as delivered. Without this the retirement is a rule the pipeline breaks on every call |
| 18A | Variant choice removed from step 2 — the only decision left is GPT Image or Nano Banana. Rules 1 and 2 rewritten |
| 44 | Defaults **19** and **47** rewritten |
| D, E1, E7, E10 | Two rationale rows; QA row now fails off-arsenal models; the Flare call template is gone; lint bans `flare` and `nano_banana_flash` outside retirement notices |
| Open Decisions | The 17 Sep test log is marked superseded, and the one pair where Flare was picked as more realistic is recorded as overturned rather than deleted |

**Origin:** user decision after the street-interview runs. Twenty jobs this session logged `nano_banana_flash` against a passed `nano_banana_2`, which made the gap between what the document routes and what the pipeline runs impossible to ignore.

**Scope:** immediate. Frames already approved keep their approval; the ledger entry for any of them that logged flash is marked as such rather than rewritten.

---

# CHANGELOG — V7.51.1 → V7.51.2 *(cut authorised)*

| § | Change |
|---|---|
| **16B** | **Generation Call Labelling (new).** §16 governed the prompt and §26 governed the prompt's header; nothing governed the tool-call stream. Every call now carries its beat ID and script line immediately above it, every batch carries a manifest whose order matches the payload item for item, and every returned job id is written back against its beat. A bare job id is undelivered |
| 16A | Routing table gains a row: generation calls and job ids are prose, never a widget |
| 26 | Item 1 extended — the same label travels to the call, not just the prompt block |
| 44 | New default **71** |
| D, E10 | One rationale row; lint now checks that every ledger job id resolves to a beat and that batch manifests match their payloads |

**Origin:** a build run through the connectors is read as a column of model strings and job ids. The prompt was always delivered correctly; the *call* was not labelled at all, so at eighty beats nothing in the stream said which beat was which — and an id that cannot be traced to a beat cannot be scored, ledgered or reissued.

**Scope:** immediate, and it changes nothing about what is generated. One line per call.
